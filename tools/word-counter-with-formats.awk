...
#
# This script counts words, calculating absolute frequency, relative frequency, and word format.
# 
...
# Usage:
#
#     gawk -f word-counter-with-format.awk input.txt > output.txt
#     gawk -f word-spacer.awk input.txt | gawk -f word-counter-with-format.awk > output.txt
#
...
function insert(token) {
	total++;
	counters[token]++;
	if (!(token in formats)) formats[token]=format(token);
}
...
END {
	OFS="\t"
	print "TOKEN", "COUNT", "FRAC", "FORMAT";
	for (token in counters) {
		count = counters[token];
		frac = counters[token] / total;
		print token, count, frac, formats[token] | "sort -t'	' -k1,1";
	}
}
